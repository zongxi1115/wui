...
```bash
npx @wui-design/cli@latest init       # writes wui.json + the cn() helper
npx @wui-design/cli@latest add button # copies button (+ its registry deps) into your project
```

Because the registry follows the shadcn `registry-item.json` schema, the
official CLI works too:
`npx shadcn@latest add https://ui.wzx.wang/r/button.json`.
...
claude mcp add wui -- npx -y @wui-design/mcp                      # or over stdio
```
