...
├─ packages/mcp/          # the MCP server (@wui/mcp): shared core + stdio entry
...
```bash
npx @wui/cli@latest init       # writes wui.json + the cn() helper
npx @wui/cli@latest add button # copies button (+ its registry deps) into your project
```
...
claude mcp add wui -- npx -y @wui/mcp                             # or over stdio
```
