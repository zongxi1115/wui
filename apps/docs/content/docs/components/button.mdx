...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: "boolean",
      default: "false",
      description: "加载状态。开启后加载指示器会从文案左侧平滑展开（icon 尺寸下与图标交叉淡入替换），同时禁用按钮并设置 aria-busy。使用 asChild 时仅同步状态属性，不注入指示器。",
    },
    {
      prop: "disabled",
      type: "boolean",
    ...
    },
    {
      ...
      default: '"submit"',
      description: "原生表单按钮类型，未声明时遵循浏览器默认值 submit。表单内的非提交按钮请显式声明为 button。",
    },
    {
      prop: "className",
      type: "string",
