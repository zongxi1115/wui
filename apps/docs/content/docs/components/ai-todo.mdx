  ...
  data={[
    {
      ...
      description: "标题前的图标，默认为清单图标；传入 `null` 可隐藏。",
    },
  ]}
/>
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      default: '"pending"',
      description: "任务当前的生命周期状态。pending 为待执行，in-progress 为执行中（带旋转动效与行高亮），completed 为已完成（勾选描边绘制、删除线从左向右划过），cancelled 为已取消。",
    },
    {
      prop: "onStatusChange",
      type: "(status: AiTodoStatus) => void",
      description: "当传入该回调时，左侧状态图标会变成可交互按钮，点击可在 completed 与 pending 之间切换；未传入时渲染为带状态文案的只读图标。",
    },
    {
      prop: "className",
      type: "string",
